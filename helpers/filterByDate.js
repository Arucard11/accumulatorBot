  ...
  if (tokenAge === "day") {
    ...
    return (filteredArray = tokenArray.filter(
      (token) => {
        ...
        return tokenDate >= oneDayAgo && tokenDate < now && token.daily_volume !== null && token.daily_volume > 20000
      }
    ));
  }
  if (tokenAge === "week") {
    ...
    return (filteredArray = tokenArray.filter(
      (token) => new Date(token.minted_at) > oneWeekAgo && token.minted_at!== null && token.daily_volume !== null && token.daily_volume > 20000
    ));
  }
  if (tokenAge === "month") {
  
    return (filteredArray = tokenArray.filter(
      (token) => new Date(token.minted_at) > oneMonthAgo && token.minted_at!== null && token.daily_volume !== null && token.daily_volume > 20000
    ));
  }
  if (tokenAge === "three months") {
    
    return (filteredArray = tokenArray.filter(
      (token) => new Date(token.minted_at) > threeMonthsAgo && token.minted_at!== null && token.daily_volume !== null && token.daily_volume > 20000
    ));
  }
  if (tokenAge === "six months") {
    
    return (filteredArray = tokenArray.filter(
      (token) => new Date(token.minted_at) > sixMonthsAgo && token.minted_at !== null && token.daily_volume !== null && token.daily_volume > 20000
    ));
  }
  if (tokenAge === "year") {
    
    return (filteredArray = tokenArray.filter(
      (token) => new Date(token.minted_at) > oneYearAgo && token.minted_at !== null && token.daily_volume !== null && token.daily_volume > 20000
    ));
  }
